    ...
    >
      <div className="p-6 md:p-8 lg:p-10 relative z-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.2em] text-ink-500">
            <span className="text-accent">{project.category}</span>
            <span>·</span>
            <span>{project.year}</span>
          </div>
          <ArrowUpRight className="w-5 h-5 text-ink-500 group-hover:text-accent group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all" />
        </div>
        ...
        <div className={large ? "grid grid-cols-1 lg:grid-cols-12 gap-8" : ""}>
          <div className={large ? "lg:col-span-7" : ""}>
            <h3
              className={`font-serif text-ink-50 mb-4 leading-[1] tracking-tight ${
                large ? "text-5xl md:text-6xl" : "text-3xl md:text-4xl"
              }`}
            >
              {project.title}
            </h3>
            <p className="text-ink-300 text-base md:text-lg leading-relaxed mb-6 text-pretty">
              {project.tagline}
            </p>
          </div>

          {/* Metric card for large variant */}
          {large && (
        ...
        {/* Bottom CTA row */}
        <div className="flex items-center justify-between pt-6 border-t border-ink-800 mt-6">
...
function SecondaryCard({ project }: { project: (typeof projects)[0] }) {
  ...
  return (
    <div
      role="article"
      onClick={() => router.push(`/projects/${project.slug}`)}
      className="group p-6 border border-ink-800 bg-ink-900/20 hover:border-accent/30 hover:bg-ink-900/40 transition-all duration-300 cursor-pointer"
    >
